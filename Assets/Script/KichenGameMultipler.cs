...
public class KichenGameMultipler : NetworkBehaviour
{
    ...
    private void Awake()
    {
        ...
        DontDestroyOnLoad(gameObject); // позвол€ет не удал€ть игрока 
    ...
    private void NetworkManager_ConnectionApprovalCallback(NetworkManager.ConnectionApprovalRequest connectionApprovalRequest, NetworkManager.ConnectionApprovalResponse connectionApprovalResponse)
    {
        if(SceneManager.GetActiveScene().name != Loader.Scene.CharacterSelectScene.ToString())
        {
            ...
            connectionApprovalResponse.Reason = "игра уже началась";
            return;
        ...
        if(NetworkManager.Singleton.ConnectedClientsIds.Count >= MAX_PLAYER_AMOUNT)
        {
            Debug.Log("2");
            connectionApprovalResponse.Approved = false;
            connectionApprovalResponse.Reason = "лоби уже заполнено";
            return;
        }
